...
namespace TechMove.Controllers
{
    public class ServiceRequestsController : Controller
    {
        ...
        public async Task<IActionResult> Create()
        {
            ...
            var activeContracts = await _apiService.GetContractsAsync(status: "Active");

            if (!activeContracts.Any())
            {
                // no active contracts exist - no point showing the form
                TempData["ErrorMessage"] =
                    "No active contracts found. Please activate a contract first.";
        ...
        public async Task<IActionResult> Create(ServiceRequest request)
        {
            ...
            // workflow guard now handled by the API
            // API returns error message if contract is Expired or On Hold
            var (success, error) = await _apiService.CreateServiceRequestAsync(request);

            if (!success)
            {
                // show the error from the API to the user
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            ViewBag.StatusOptions = new SelectList(
                new[] { "Pending", "In Progress", "Completed" }, request.Status);
            return View(request);
        }
        ...
        public async Task<IActionResult> Edit(int id, ServiceRequest request)
        {
            ...
            await _apiService.UpdateServiceRequestAsync(request);
            return RedirectToAction("Index");
        }
